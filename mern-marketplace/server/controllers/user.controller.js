...
const stripeCustomer = async (req, res, next) => {
  try {
    const { products, tip = 0 } = req.body.order || {}
    ...
    const totalAmount = subtotal + Number(tip)  
    const amountInCents = Math.round(totalAmount * 100)

    console.log(`🛒 Subtotal: $${subtotal.toFixed(2)} | 💰 Tip: $${tip} | 💳 Total: $${totalAmount.toFixed(2)} (${amountInCents}¢)`)
    ...
    const charge = await myStripe.charges.create({
      ...
      description: `Order total $${totalAmount.toFixed(2)} (incl. $${tip} tip)`
    })


    req.body.order.payment_id = charge.id
    
    next()
  } catch (err) {
    console.error('Stripe charge error:', err.message)
    return res.status(400).json({
      error: err.message || 'Payment failed - card declined'
    })
  }
}
