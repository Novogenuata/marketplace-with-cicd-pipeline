...
const useStyles = makeStyles(theme => ({
  ...
  margin: '10px 0',
  padding: '10px',
  backgroundColor: 'rgba(72, 175, 148, 0.1)',
  borderRadius: '4px'
}
...
export default function Order({match}) {
  ...
  const getTipTotal = () => order.tip || 0

  const getGrandTotal = () => getTotal() + getTipTotal()
  ...
  <span className={classes.total}>Subtotal: ${getTotal().toFixed(2)}</span>
  {order.tip > 0 && (
    <span className={classes.total} style={{color: 'rgb(72, 175, 148)'}}>
      Tip: +${getTipTotal().toFixed(2)}
    </span>
  )}
  <span className={classes.total} style={{fontSize: '1.4em', color: 'rgb(53, 97, 85)'}}>
